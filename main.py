...
mobile_chatbot_visibility_score=[]
pc_chatbot_visibility_score = []
...
for row in data:
    ...
    chatbot_visibility_score = parse_score(row["La du merke til chatbotten i høyre hjørne?"])
    ...
    if tool_used == 'mobil' or tool_used == 'mobile':
        ...
        if chatbot_visibility_score is not None:
            mobile_chatbot_visibility_score.append(chatbot_visibility_score)
    elif tool_used == 'pc':
        if navigation_score is not None:
            x+=1
            if x > 18:
        ...
        if chatbot_visibility_score is not None:
            pc_chatbot_visibility_score.append(chatbot_visibility_score)
...
avg_mobile_chatbot_visibility_score, std_mobile_chatbot_visibility_score = calculate_avg_and_std(mobile_chatbot_visibility_score)
avg_pc_chatbot_visibility_score, std_pc_chatbot_visibility_score = calculate_avg_and_std(pc_chatbot_visibility_score)
...
mobile_avgs = [
    ...
    avg_mobile_interactive_elements, avg_mobile_chatbot_visibility_score
]
pc_avgs = [
    avg_pc_navigation, avg_pc_visual, avg_pc_responsiveness,
    avg_pc_layout, avg_pc_images_fonts, avg_pc_design_improvement,
    avg_pc_interactive_elements, avg_pc_chatbot_visibility_score
]
mobile_stds = [
    ...
    std_mobile_interactive_elements, std_mobile_chatbot_visibility_score
]
pc_stds = [
    std_pc_navigation, std_pc_visual, std_pc_responsiveness,
    std_pc_layout, std_pc_images_fonts, std_pc_design_improvement,
    std_pc_interactive_elements, std_pc_chatbot_visibility_score
]
...
ax.set_ylabel('Gjennomsnittlig score', fontsize=18)
ax.set_xticks(x)
